...
### Приоритеты переменных с одинаковыми именами в порядке убывания:
- Переданные параметром запуска из CLI
- локальные переменные
- глобальные в секции `vars:`
- глобальные в секции `env:`
- переменные окружения, определенные в ОС  

### Сводная таблица присваивания

| Метод      | Где работает     | Тип значения           | Пример                       |
| ---------- | ---------------- | ---------------------- | ---------------------------- |
| **Прямое** | `vars`, `env`    | Любой YAML             | `KEY: value`                 |
| **Shell**  | `vars`           | Строка (вывод команды) | `KEY: { sh: "echo hi" }`     |
| **Файл**   | `vars`           | Строка (содержимое)    | `KEY: { file: "txt" }`       |
| **JSON**   | `vars`           | Объект (парсинг)       | `KEY: { json: "file.json" }` |
| **Шаблон** | `vars`, `env`    | Строка (результат)     | `KEY: "{{.OTHER}}"`          |
| **CLI**    | Командная строка | Строка                 | `task name KEY=val`          |

### Встроенные переменные Task

#### Специальные переменные Task

| Переменная              | Описание                                                                                               | Пример значения              |
| ----------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------- |
| **`{{.TASK}}`**         | Имя текущей выполняемой задачи.                                                                        | `build`, `deploy`, `default` |
| **`{{.ROOT_DIR}}`**     | Абсолютный путь к корневой директории проекта (где лежит главный `Taskfile.yml`).                      | `/home/user/project`         |
| **`{{.TASKFILE_DIR}}`** | Абсолютный путь к директории, где лежит текущий файл `Taskfile.yml`. **Важно для модулей (includes).** | `/home/user/project/backend` |
| **`{{.WORKING_DIR}}`**  | Директория, в которой фактически выполняется команда (может отличаться при использовании `dir:`).      | `/home/user/project`         |
| **`{{.OS}}`**           | Название операционной системы.                                                                         | `linux`, `windows`, `darwin` |
| **`{{.ARCH}}`**         | Архитектура процессора.                                                                                | `amd64`, `arm64`, `386`      |
| **`{{.NUM_CPU}}`**      | Количество логических ядер процессора.                                                                 | `8`, `12`                    |
| **`{{.CLI_ARGS}}`**     | Аргументы, переданные после `--` в командной строке.                                                   | `--verbose --force`          |

#### Переменные окружения системы

Task автоматически наследует **все переменные окружения  операционной системы**. Вы можете обращаться к ним напрямую через шаблонизатор.


#### `vars:` (Переменные задач Task)
- **Имеют преимущество перед env: в случае конфликта имен**
...
- **Имеют преимущество перед заведенными в ОС: в случае конфликта имен**
