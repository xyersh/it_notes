...
Если контейнеры создавались с указанием общей сети - они могут общаться друг с другом также посредством их DNS-имен (то что указано параметром `--name` при создании контейнера)
