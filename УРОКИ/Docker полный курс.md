...
- docker system prune [-a] - Удаляет только `<none>` образы + остановленные контейнеры + неиспользуемые сети + висячий кэш
	- `-a` - Удаляет **все образы**, на которые не ссылается ни один контейнер (даже если у них есть теги, например `myapp:latest`), плюс всё остальное.

- docker builder prune [-a] - Удаляет только висячие слои кэша
		- `-a` - Удаляет **весь кэш сборки**, включая те слои, которые потенциально могут ускорить будущие сборки.
