...
| Хинт                         | Назначение                                                                   | Зона применимости                                                                                                                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`USE_NL(table_alias)`**    | Принуждает использовать соединение **Nested Loops** (Вложенные циклы).       | Идеально для **OLTP-систем**. Эффективно, когда одна из таблиц маленькая, а на столбце соединения другой таблицы есть эффективный индек                                                      |
| **`USE_HASH(table_alias)`**  | Принуждает использовать соединение **Hash Join** (Хеш-соединение).           | Идеально для **OLAP/DWH-систем**. Эффективно для соединения двух больших таблиц. В параметре хинта указывается таблица, которая будет хэшироваться для дальнейшего ее соединения с `probe`-таблицей  |
| **`USE_MERGE(table_alias)`** | Принуждает использовать соединение **Sort-Merge Join** (Сортировка-Слияние). | Используется редко. Может быть полезен, когда данные уже отсортированы или для больших объемов, где неэффективен `HASH JO                                                                    |
| **`NO_USE_NL(table_alias)`** | Запрещает использование соединений **Nested Loops**.                         | Принуждение к `HASH` или `MERGE` при неэффективности                                                                                                                                         |
...
| `DRIVING_SITE(<tbl_alias>)`        | При соединениях по db-link указывает на какой стороне выполнять соединение (указывается таблица с **наибольшим** количеством строк отбираемых в запросе) | Управление методом соединения в специфических конструкциях.                                                                                |
NO_SWAP_JOIN_INPUTS - запретить оптимизатору менять роли таблиц при объединении хэшированием
