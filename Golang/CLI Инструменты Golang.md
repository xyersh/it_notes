...
### go work
Объединяет модули выбранных зависимостей в единый воркспейс, позволяя пользоваться ими локально без использования **go get** и скачивания зависимостей из сети.
Это очень удобно когда нужно проверить работу одного модуля после изменения другого. Избавляет от необходимости пушей  в репозиторий и использования **replace** в **go.mod**.

При создании воркспейса создаются следующие  файлы:
- **go.work** - хранит список модулей
- **go.work.sum** - хранит хэш-суммы модулей


#### Команды:
- `go work init [mod1 mod2...]` - инициализация воркспейса, где mod1, mod2- модули которые добавляются к воркспейсу.  В результате команды  создается файл **go.work**
- `go work use <path>`- добавление модуля к воркспейсу по указанному пути. Путь относительно корня воркспейса
- `go work use -r <path>` - удаление модуля из воркспейса
- `go work sync` - синхронизирует модули воркспейса. Обновляет файл **go.mod.sum**, добавляя туда актуальные хэш-суммы зависимостей.
- `go work edit -json` - выводит структуру файла **go.work** в JSON-формате.

#### Best Practices
- **НИКОГДА не коммитьте `go.work` в Git**. Работа с ворксрейсом - это исключительная пререгатива локальной разработки (у каждого разработчика на локальной машине может быть свой воркспейс со своими зависимостями)
- **Не используйте `replace` в `go.mod` для локальных путей**. Если вам нужно связать два локальных модуля, используйте `go.work`. Директива `replace` в `go.mod` должна использоваться только для форков внешних библиотек.
- **Гранулярность модулей**. Не делайте воркспейс из 50 модулей — это замедлит работу `gopls` и индексацию. Объединяйте в воркспейс только те модули, над которыми вы работаете _прямо сейчас_.
### go build
...
### go fix 
Правка существующего кода согласно современным требованиям к синтаксису.
```bsh
go fix ./...   # правки в .go-файлах в всех поддиректориях относительно кекущей папки
go fix -diff ./... # предврительный просмотр правок без последующего изменения кода 
```
