...
- Никогда не встраивать мьютекс в структуру. Мьютекс должен быть отдельным полем структуры. В противном случае структура наследует методы мью текса, и следовательно возможно ее использование не по назначению. 
...
```
 - Никогда не хранить ссылку на мьютекс в поле структуры.  Плохой пример:
 ```go
 type SomeStruct struct {
	mu *sync.Mutex
}
