...
## Предустановленные ошибки пакета sql

- `ErrConnDone` - возникает при любых операциях с БД когда подключение к  БД уже закрыто
- `ErrNoRows` - возникает при получении значении из пустой выборки при использовании `QueryRow()`
- `ErrTxDone` - возвращается при проведении любой операции на  транзакции, которая уже была подтверждена, или откачена.



##  Подключение и настройка пула соединений
| Функция                                               | Описание                                                                                 |
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `sql.Open(driverName string, DNS string)(*DB, error)` | Валидирует драйвер и создаёт объект пула. **Не открывает сеть**.                         |
| `(db *DB)Ping() error`                                | Проверяет, что подключение к БД активно. Рекомендуется вызывать сразу после `Open`.      |
| `(db *DB)SetMaxOpenConns(n int)`                      | Определяет максимальное количество открытых соединений к БД                              |
| `(db *DB)SetMaxIdleConns(n int)`                      | Определяет максимальное кол-во простаивающих соединений к БД                             |
| `(db *DB)SetConnMaxLifetime(t time.Time)`             | Максимальное кол-во времени, в которое соединение в БД может быть переиспользовано       |
| `(db *DB)SetMaxIdleConns(t time.Time)`                | Максимальное кол-во времени, которое соединение может провести в простаивающем состоянии |
|                                                       |                                                                                          |
...
| Функция                                                                             | Описание                                                                      |     |
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | --- |
| `(db *DB)Query(query string, args ...any)(*Rows, error)`                            | Выполняет запрос `SELECT`                                                     |     |
| `(db *DB)QueryContext(ctx ontext.Context, query string, args ...any)(*Rows, error)` | Выполняет запрос с использованием контекста                                   |     |
| `(r *Rows)Next() bool`                                                              | Получение следующей строки из выборки                                         |     |
| `(r *Rows)Scan(dest ...any) error`                                                  | Копирует значения колонок из конкретной строки в переменные указанные в`dest` |     |
| `(r *Rows)Close()`                                                                  | Закрывает resultSet. Обязателен во избежание утечки памяти                    |     |
|                                                                                     |                                                                               |     |
...
| Функция                                             | Описание                                                                                        |     |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------- | --- |
| `(db *DB)QueryRow(query string, args ...any)(*Row)` | Выполнение запроса, который возвращает не более 1 строки.                                       |     |
| `(r *Row)Scan(dest ...any) error`                   | Копирует результаты из колонок единственной строки выборки. Если пусто - вернёт `sql.ErrNoRows` |     |
|                                                     |                                                                                                 |     |
