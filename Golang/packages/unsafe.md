...
- `unsafe.Sizeof(v T) uintptr`: Возвращает размер в байтах, который занимает переменная `v` в памяти (в байтах).
 ...
 - `unsafe.Add(ptr Pointer, delta int) Pointer` - смещает место указания поитера на значние `delta`. Позволяет избавится от неуклюжей кострукции вида: `ptr = unsafe.Pointer(uintptr(ptr) + uintptr(delta))`
