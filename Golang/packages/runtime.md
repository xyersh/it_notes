...
- `runtime.NumGoroutine()`- Возвращает текущее количество запущенных горутин.
...
- **`runtime.move2heap(data any)`** - аллоцирует переменную из стека в кучу. Полезно для случаев оптимизации, связанной с ограничением колличества аллокаций.
### Примеры
...
#### `runtime.GC()`: Вызывает сборку мусора.
 * Пример:
```go
package main
