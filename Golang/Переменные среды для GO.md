...
- `GOFLAGS` - это переменная окружения в языке программирования Go, которая содержит список флагов, автоматически применяемых ко всем вызовам команд `go build`, `go test`, `go get` и другим подкомандам утилиты `go`. Она избавляет от необходимости вручную прописывать одни и те же ключи при каждом