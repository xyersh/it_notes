### [[JSON - база]]
Целевой пакет `encoding/json`

### Глоссарий 
- **Сериализация(маршалинг)** - процесс конвертирования объекта структуры в любой тип, позволяющий хранить и передавать эти данные по сети (JSON, или любой другой в представлении  байтового массива) 
- **Десериализация (анмаршалинг)** - процесс преобразования данных из любого типа, рассчитанного на хранение и передачу (JSON, или любой другой в представлении  байтового массива) в объекты структур.
- **структурные теги** - 
### Маршалинг
`json.Marshal(v any) ([]byte, error)`.
Где:
- `v` - структура-источник данные которой подлежат сериализации
- `[]byte` - получаемый сериализованный JSON-объект в байтовой нотации
- `error` - ошибка при сериализации
...
- В случае отсутствия json-тегов у полей структуры - поля JSON (ключи) должны точно совпадать с экспортируемыми полями Go-структуры по имени И регистру.
