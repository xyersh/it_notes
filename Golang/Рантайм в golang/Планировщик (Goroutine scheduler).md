...
- **Ожидание на канале (`channel`)**: Если горутина блокируется при отправке или получении данных через канал, она помещается в одну из очередей ожидания (например, `sendq` или `recvq`), которые являются частью **самой структуры канала**.
		-  **Где находятся:** У каждого канала есть два связанных списка: `sendq` (очередь отправителей) и `recvq` (очередь получателей). Эти списки состоят из структур типа `sudog`.
		- **Сущность `sudog`:** Это специальная обертка над горутиной (`g`), которая позволяет одной горутине находиться в нескольких списках ожидания одновременно (например, в конструкции `select`).
- **Ожидание на примитивах синхронизации (`sync.Mutex`, `sync.WaitGroup`, `sync.Cond`)**: Горутина помещается в список ожидания, который является частью соответствующей структуры синхронизации.
	- **Где находятся:** В глобальной хеш-таблице **semaRoot**.
	- **Как это работает:** Горутина упаковывается в тот же `sudog` и вешается в сбалансированное дерево ожидания, привязанное к адресу переменной (семафору).
- **Блокирующий системный вызов (Syscalls)**:
    ...
    - Для **синхронного I/O** (например, чтение из файла), **M** (поток ОС) может быть заблокирован. В этом случае Go-планировщик может отсоединить **P** от этого **M** и создать новый **M** или использовать свободный **M** из тред-пула для выполнения других горутин, которые были запланированы на этом **P**. Заблокированная горутина остается связанной с этим **M** до завершения системного вызова. (`Handoff`) 
...
- **Повторное Использование (Pooling/Кэширование):**
    ...
    - Вместо этого он возвращается в **кэш/пул бездействующих OS-потоков** (`tread pool`) для повторного использования. Это позволяет Go избегать дорогостоящих системных вызовов для создания и уничтожения OS-потоков при каждой новой задаче, что значительно повышает производительность.
